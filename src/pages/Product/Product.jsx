          ...
          <div className="container-fluid">
            <div className="row">
              <div className="col-md-4 p-5 column-coupon">
                ...
                <button type="button" class="btn btn-choco-coupon mt-5">Apply Coupon</button>
              ...
              <div className="col-md-8 p-5 column-products">
                ...
                <div class="row mt-5 justify-content-center">
                  {
                    this.getCoffee("1") ? this.product.coffee.map((item) => {
                      return (
                        <CardProduct image={item.image} title={item.name} price={item.price} key={item.id} />
                      )
                    })
                      : this.getNonCoffee("2") ? this.product.nonCoffee.map((item) => {
                        return (
                          <CardProduct image={item.image} title={item.name} price={item.price} key={item.id} />
                        )
                      })
                        : this.getFoods("3") ? this.product.nonCoffee.map((item) => {
                          return (
                            <CardProduct image={item.image} title={item.name} price={item.price} key={item.id} />
                          )
                        }) : null
